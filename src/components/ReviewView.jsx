...
const ReviewView = ({ assets, combinedHistory, loadArchiveMonth }) => {
  ...
  const stats = useMemo(() => {
    ...
    records.forEach(r => {
      if (r.type === 'income') {
        totalIncome += r.total;
        if ((r.payer || '').includes('大狗狗🐕') || (r.payer || '').includes('用戶1')) incomeUserA += r.total;
        else if ((r.payer || '').includes('阿陞🐶') || (r.payer || '').includes('用戶2')) incomeUserB += r.total;
        ...
        if (r.type === 'spend') {
          ...
          if ((r.payer || '').includes('大狗狗🐕') || (r.payer || '').includes('用戶1')) { expenseUserA += r.total; userAItems.push(r); }
          else if ((r.payer || '').includes('阿陞🐶') || (r.payer || '').includes('用戶2')) { expenseUserB += r.total; userBItems.push(r); }
        }
