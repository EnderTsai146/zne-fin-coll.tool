...
function App() {
  ...
  useEffect(() => {
    ...
    creditCards.forEach(card => {
      ...
      // Check 1: 5-Day Prior Reminder Notification
      const dueThisMonth = new Date(currentYear, currentMonth, bDay);
      const diffTime = dueThisMonth.getTime() - today.getTime();
      const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));

      if (card.autoPay && diffDays === 5 && unpaidAmount > 0) {
        const linkedBank = card.linkedBankAccountId 
          ? newAccounts.find(a => a.id === card.linkedBankAccountId)
          : null;
        const linkedBankName = linkedBank ? linkedBank.nickname : '綁定活儲';
        const isBalanceSufficient = linkedBank ? linkedBank.balance >= unpaidAmount : false;

        if (!isBalanceSufficient) {
          customAlert(
            `⚠️ 【自動扣款預警】信用卡【${card.nickname}】將於 5 天後 (${dueThisMonth.toISOString().slice(0, 10)}) 自動扣繳 $${unpaidAmount.toLocaleString()} ${card.currency || 'TWD'}。\n\n綁定帳戶【${linkedBankName}】當前餘額 ($${(linkedBank?.balance || 0).toLocaleString()}) 不足！請盡速補足資金以利順利扣款！`,
            "自動扣款餘額預警"
          );
        }
      }

      // Check 2: Auto-Pay Due Date Execution
      if (card.autoPay && currentDay === bDay) {
        if (unpaidAmount === 0) {
          console.log(`AutoPay skipped for ${card.nickname}: balance is $0`);
          return;
        }
        ...
        if (!linkedBank) {
          customAlert(`⚠️ 信用卡【${card.nickname}】自動扣款失敗：未設定或找不到綁定扣款活儲帳戶！`, "扣款失敗");
          return;
        }

        if (linkedBank.balance < unpaidAmount) {
          customAlert(`❌ 信用卡【${card.nickname}】今日自動扣款失敗：綁定帳戶【${linkedBank.nickname}】餘額不足 ($${linkedBank.balance.toLocaleString()} < $${unpaidAmount.toLocaleString()})，請補足資金。`, "扣款失敗");
          return;
        }

        newAccounts = newAccounts.map(a => {
          if (a.id === linkedBank.id) return { ...a, balance: a.balance - unpaidAmount };
          if (a.id === card.id) return { ...a, balance: a.balance + unpaidAmount };
          return a;
        });
        ...
        newTxRecords.push({
          ...
          type: 'transfer',
          category: '資產劃撥',
          total: card.currency === 'TWD' ? unpaidAmount : Math.round(unpaidAmount * (currentFxRate || 31.5)),
          sourceAmount: unpaidAmount,
          targetAmount: unpaidAmount,
          payer: card.owner === 'joint' ? '共同帳戶' : (card.owner === 'userA' ? '大狗狗🐕' : '阿陞🐶'),
          accountId: linkedBank.id,
          targetAccountId: card.id,
          note: `🤖 [自動扣繳結清] 信用卡: ${card.nickname} (自 ${linkedBank.nickname} 劃撥)`,
          timestamp: new Date().toISOString()
        });

        customAlert(`🤖 【自動扣繳成功】信用卡【${card.nickname}】今日已成功自【${linkedBank.nickname}】自動劃撥扣繳 $${unpaidAmount.toLocaleString()} ${card.currency || 'TWD'} 結清帳單！`, "自動扣款成功");
      }
    });

    localStorage.setItem('autoPayLastCheckedDate', todayStr);
